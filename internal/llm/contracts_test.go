...
func TestDecodeSelection(t *testing.T) {
	...
	assertSelectionError(t, opts, `{"schema_version":1,"thread_actions":[{"thread_id":"thread-1","decision":"summarize_and_resolve"}]}`, "safe_to_resolve_rationale")
	assertSelectionError(t, SelectionOptions{KnownThreads: map[string]bool{"t1": true, "t2": true}, MaxResolvedThreads: 1}, `{"schema_version":1,"thread_actions":[{"thread_id":"t1","decision":"summarize_and_resolve","safe_to_resolve_rationale":"a"},{"thread_id":"t2","decision":"summarize_and_resolve","safe_to_resolve_rationale":"b"}]}`, "cap")
