...
func TestEstimateUSDKnownModel(t *testing.T) {
	...
	cost, ok := EstimateUsageUSD("claude-sonnet-4-6", Usage{
		TokensIn: p(1_000_000), TokensOut: p(1_000_000), CacheRead: p(1_000_000), CacheCreate5m: p(1_000_000),
	})
	if !ok {
...
func TestEstimateUSDNilTokensAreZero(t *testing.T) {
	cost, ok := EstimateUsageUSD("claude-opus-4-8", Usage{})
	if !ok || cost != 0 {
...
func TestEstimateUSDCurrentClaudeRates(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cost, ok := EstimateUsageUSD(tt.model, Usage{TokensIn: p(tt.input), TokensOut: p(tt.output), CacheRead: p(tt.cacheRead)})
			if !ok {
...
func TestEstimateUsageUSDPricesEachSonnetBucketIndependently(t *testing.T) {
	tests := []struct {
		...
		{name: "input", usage: Usage{TokensIn: p(1_000_000)}, want: 2},
		{name: "output", usage: Usage{TokensOut: p(1_000_000)}, want: 10},
		{name: "cache read", usage: Usage{CacheRead: p(1_000_000)}, want: 0.2},
		{name: "five minute cache write", usage: Usage{CacheCreate5m: p(1_000_000)}, want: 2.5},
		{name: "one hour cache write", usage: Usage{CacheCreate1h: p(1_000_000)}, want: 4},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
