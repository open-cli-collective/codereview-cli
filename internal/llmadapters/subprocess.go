...
var _ llm.Adapter = (*SubprocessAdapter)(nil)
...
func (a *SubprocessAdapter) startClaudeBG(ctx context.Context, req Request, resumeSessionID string) (Stream, error) {
	...
	go stream.runClaudeBG(process.Context(), a, process.Command(), process.Stdout(), process.Stderr(), scratch, workDir)
	return stream, nil
}
...
func (w subprocessLogWriter) Write(p []byte) (int, error) {
	...
	return w.stream.WithLogFile(func(logFile *os.File) (int, error) {
		if w.stream.logBytesLeft == 0 {
			if !w.stream.logCapped {
				if _, err := logFile.Write([]byte("warning: reviewer workspace tool output cap reached; further subprocess logs truncated\n")); err != nil {
					return 0, err
				}
		...
		n, err := logFile.Write(writeBytes)
		if w.stream.logBytesLeft > 0 {
			w.stream.logBytesLeft -= n
		}
		if w.stream.logBytesLeft == 0 && !w.stream.logCapped {
			if _, writeErr := logFile.Write([]byte("warning: reviewer workspace tool output cap reached; further subprocess logs truncated\n")); writeErr != nil && err == nil {
				err = writeErr
			}
			w.stream.logCapped = true
		}
