...
func validateArtifactPath(layout statepaths.Layout, artifactPath string) error {
	...
	if err := rejectSymlinkedComponents(root, artifact); err != nil {
		return err
	}
	return nil
}

func rejectSymlinkedComponents(root, artifact string) error {
	rel, err := filepath.Rel(root, artifact)
	if err != nil {
		return err
	}
...
func rejectSymlink(path string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("%s contains a symlink component", path)
	}
	return nil
}
