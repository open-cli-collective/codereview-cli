...
| config.secrets.profiles.<name>.backend.onepassword.timeout | Not yet editable in interactive init. Non-secret timeout for 1Password-backed secrets profiles. | `cr config secrets-profile set --op-timeout`; later secrets-management workflow tickets. | Empty input normalizes to the current service-account/desktop default timeout when that backend family requires one. Existing value is pre-populated. | Preserve on skip. Overwrite validates as a Go duration string. Switching away from 1Password clears the field from config. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.vault_id | Not yet editable in interactive init. Non-secret 1Password vault selection. | `cr config secrets-profile set --op-vault-id`; later secrets-management workflow tickets. | Required for `op`, `op-connect`, and `op-desktop`. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Switching away from 1Password clears the field from config. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.item_title_prefix | Not yet editable in interactive init. Optional 1Password item naming override. | `cr config secrets-profile set --op-item-title-prefix`; later secrets-management workflow tickets. | Empty means use the underlying keyring library default. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Switching away from 1Password clears the field from config. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.item_tag | Not yet editable in interactive init. Optional 1Password item tag override. | `cr config secrets-profile set --op-item-tag`; later secrets-management workflow tickets. | Empty means use the underlying keyring library default. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Switching away from 1Password clears the field from config. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.item_field_title | Not yet editable in interactive init. Optional 1Password item-field title override. | `cr config secrets-profile set --op-item-field-title`; later secrets-management workflow tickets. | Empty means use the underlying keyring library default. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Switching away from 1Password clears the field from config. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.connect_host | Not yet editable in interactive init. Non-secret 1Password Connect endpoint. | `cr config secrets-profile set --op-connect-host`; later secrets-management workflow tickets. | Required for `op-connect`. Ignored for other backend kinds. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Backend switches clear it when no longer relevant. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.connect_token_env | Not yet editable in interactive init. Non-secret env-var name for the 1Password Connect token. | `cr config secrets-profile set --op-connect-token-env`; later secrets-management workflow tickets. | Defaults to `OP_CONNECT_TOKEN` for `op-connect`. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Backend switches clear it when no longer relevant. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.service_token_env | Not yet editable in interactive init. Non-secret env-var name for the 1Password service-account token. | `cr config secrets-profile set --op-service-token-env`; later secrets-management workflow tickets. | Defaults to `OP_SERVICE_ACCOUNT_TOKEN` for `op`. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Backend switches clear it when no longer relevant. | #296 adds schema, validation, runtime mapping, and command coverage. |
| config.secrets.profiles.<name>.backend.onepassword.desktop_account_id | Not yet editable in interactive init. Optional explicit 1Password desktop account id. | `cr config secrets-profile set --op-desktop-account-id`; later secrets-management workflow tickets. | Optional for `op-desktop`; empty means the runtime may fall back to `OP_DESKTOP_ACCOUNT_ID`. Existing value is pre-populated. | Preserve on skip. Overwrite trims whitespace. Backend switches clear it when no longer relevant. | #296 adds schema, validation, runtime mapping, and command coverage. |
