...
live in [docs/init-ux-contract.md](/Users/rianjs/dev/codereview-cli/docs/init-ux-contract.md).
...
| config.profiles.<name>.llm.reviewer_model_tier | Core profile wizard offers unset/default or one of small, medium, large. | `cr init --llm-reviewer-model-tier` and `--clear-llm-reviewer-model-tier`, because no config command currently mutates this field directly. | Omitted means effective small baseline. Existing value is pre-populated. | Preserve on skip. Reset clears field. Set validates model tier. | #180 prompt tests. #187 flag persistence tests. |
